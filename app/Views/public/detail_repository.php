...
if ($hasFile) {
    $fileUrl = rtrim($base_url, '/') . '/repository/' . rawurlencode($repo['slug'] ?? '') . '/download';
}
...
// --- SEO SCHEMA (sanitasi untuk JSON-LD) ---
$titleSafe    = strip_tags($repo['judul'] ?? 'Dokumen Akademik');
$abstractSafe = strip_tags($repo['abstrak'] ?? '');
$authorsSafe  = array_map(fn($a) => ["@type" => "Person", "name" => strip_tags($a['name'] ?? '')], $authorUsers);
$schemaData = [
    ...
    "author" => $authorsSafe,
    "description" => mb_strimwidth($abstractSafe, 0, 160, "...")
];
?>
    ...
    <header class="relative bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-900 text-white pb-20 md:pb-28 overflow-hidden">
        ...
        <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <nav class="flex items-center gap-2 text-xs text-emerald-200 mb-6 font-medium">
                ...
                <span>/</span>
                <span class="text-white truncate max-w-[200px]"><?= e($jenisKarya) ?></span>
            </nav>
                ...
                <?php if (!empty($keywords)): ?>
                    ...
                    <?php foreach ($keywords as $kw): ?>
                        <a href="<?= $base_url ?>/telusuri?q=<?= urlencode($kw) ?>" class="px-3 py-1.5 rounded-lg bg-slate-50 text-slate-600 text-xs font-semibold border border-slate-200 hover:border-emerald-400 hover:text-emerald-700 transition-all">
                            <span class="text-slate-300 mr-1">#</span><?= e($kw) ?>
                        </a>
                    <?php endforeach; ?>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-md shadow-slate-200/50 p-5 border border-slate-200">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="group border-t border-slate-50 pt-3">
                            ...
                            <dd class="text-sm font-semibold text-slate-800 leading-snug">
                                <?= !empty($repo['mata_kuliah']) ? e($repo['mata_kuliah']) : '-' ?>
                            </dd>
                        </div>
                                ...
                                <?php if (!empty($advisorUsers)): ?>
                                    <?php foreach($advisorUsers as $adv): ?>
                                        <div class="bg-slate-50 p-2 rounded border border-slate-100">
                                            <div class="text-sm font-bold text-slate-800"><?= e($adv['name']) ?></div>
                                            <?php if($adv['code']): ?>
                                                <div class="text-[10px] text-emerald-600 font-mono mt-0.5"><?= e($adv['code']) ?></div>
                                            <?php endif; ?>
                                        </div>
                                    <?php endforeach; ?>
                                <?php else: ?>
                            ...
                            <dd class="space-y-1">
                                <?php if (!empty($examinerUsers)): ?>
                                    <?php foreach($examinerUsers as $exm): ?>
                                        <div class="text-sm font-semibold text-slate-800"><?= e($exm['name']) ?></div>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <span class="text-sm text-slate-800">-</span>
                                <?php endif; ?>
                ...
                <div class="bg-white rounded-xl shadow-lg shadow-emerald-900/5 p-5 border border-slate-200">
                    ...
                    <div class="space-y-3">
                        ...
                        <?php if ($bookmarkable && !empty($repo['slug'])): ?>
                            <form action="<?= $base_url; ?>/bookmark/<?= e($repo['slug']) ?>/toggle" method="POST" class="w-full">
                                <input type="hidden" name="csrf_token" value="<?= e(\App\Core\Security\Csrf::token()) ?>">
