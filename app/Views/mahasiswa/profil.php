...
$base = rtrim($base_url, '/'); 
$u    = $user ?? $mhs ?? [];
$nama = $u['nama'] ?? $u['nama_lengkap'] ?? 'Mahasiswa';
$role = 'Mahasiswa';
$email = $u['email'] ?? '';
$username = $u['username'] ?? '';
$nim = $u['nim'] ?? '';
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 text-center h-full relative overflow-hidden">
                <div class="flex flex-col items-center relative z-10">
                    ...
                    <div class="mt-6 w-full border-t border-slate-100 pt-6">
                        <div class="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-3">Kredensial Login</div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 md:p-8">
                ...
                <form action="<?= $base ?>/mahasiswa/profil" method="POST" class="space-y-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" value="<?= htmlspecialchars($nim ?: '-') ?>" disabled 
                                       class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-500">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 flex justify-end gap-3 border-t border-slate-50 mt-4">
                        <a href="<?= $base ?>/mahasiswa/dashboard" class="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-colors">
                            Kembali
                        </a>
