...
<div class="space-y-4 animate-fade-in-up">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div class="space-y-1">
            ...
            <p class="text-sm text-slate-500">Kelola skripsi, jurnal, dan karya ilmiah Anda.</p>
        </div>
        <a href="<?= $base ?>/dosen/repository/create" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 hover:shadow-lg hover:shadow-emerald-600/20 transition-all active:scale-95 w-full md:w-auto">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <div class="lg:col-span-2 relative group">
                ...
                <input type="search" name="q" value="<?= e($filters['q'] ?? '') ?>" placeholder="Cari judul atau penulis..." 
            ...
            <div>
                <select name="prodi" onchange="this.form.submit()" class="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 cursor-pointer text-slate-600">
                    ...
                    <?php foreach ($prodis as $p): ?>
                        <option value="<?= e($p['id']) ?>" <?= (($filters['prodi'] ?? '') == $p['id']) ? 'selected' : '' ?>><?= e($p['nama_program_studi']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div>
                <select name="jenis" onchange="this.form.submit()" class="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 cursor-pointer text-slate-600">
                    ...
                    <?php foreach (['skripsi','tugas_akhir','jurnal','artikel','laporan','pkl','lainnya'] as $jk): ?>
                        <option value="<?= $jk ?>" <?= (($filters['jenis'] ?? '') == $jk) ? 'selected' : '' ?>><?= ucfirst(str_replace('_',' ', $jk)) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors group">
                                ...
                                <td class="px-6 py-4 align-top text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        <a href="<?= $base ?>/repository/<?= $item['slug'] ?>" target="_blank" class="p-2 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors border border-transparent hover:border-blue-100" title="Lihat Postingan">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        </a>

                                        <a href="<?= $base ?>/dosen/repository/<?= $item['id'] ?>/edit" class="p-2 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors border border-transparent hover:border-emerald-100" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </a>

                                        <form action="<?= $base ?>/dosen/my-repository/<?= $item['id'] ?>/delete" method="POST" onsubmit="return confirm('Hapus data ini secara permanen?')">
                                            <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                                            <button class="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors border border-transparent hover:border-red-100" title="Hapus">
