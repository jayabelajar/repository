...
<div class="space-y-4 animate-fade-in-up">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div class="space-y-1">
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight leading-tight">Mata Kuliah</h1>
            <p class="text-sm text-slate-600">Kelola daftar mata kuliah akademik.</p>
        </div>
        <button id="btnAddMk" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold hover:shadow-lg hover:shadow-emerald-600/20 transition-all active:scale-95">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <form class="relative group w-full" method="GET">
                ...
                <input type="search" name="q" value="<?= e($search ?? '') ?>" placeholder="Cari mata kuliah..." class="pl-10 pr-4 py-2 w-full bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php if (!empty($items)): ?>
                        <?php foreach ($items as $item): ?>
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="px-6 py-3 text-slate-900 font-semibold">
                                    <?= e($item['nama'] ?? '') ?>
                                </td>
                                ...
                                <td class="px-6 py-3 text-right">
                                    <div class="inline-flex gap-2">
                                        <button
                                            class="p-2 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors border border-transparent hover:border-emerald-100 btnEditMk"
                                            data-id="<?= $item['id'] ?>"
                                        ...
                                        <form action="<?= $base ?>/admin/mata-kuliah/<?= $item['id'] ?>/delete" method="POST" onsubmit="return confirm('Hapus mata kuliah ini?')" class="inline">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center text-slate-500">Belum ada data mata kuliah.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div id='modalMk' class='fixed inset-0 bg-black/40 backdrop-blur-sm z-50 hidden items-center justify-center px-4'>
...
<script>
(() => {
    const modal = document.getElementById('modalMk');
    const btnAdd = document.getElementById('btnAddMk');
    const btnClose = document.getElementById('closeModalMk');
    const btnCancel = document.getElementById('btnCancelMk');
    const form = document.getElementById('formMk');
    const title = document.getElementById('modalTitleMk');

    const openModal = () => { modal.classList.remove('hidden'); modal.classList.add('flex'); };
    const closeModal = () => { modal.classList.add('hidden'); modal.classList.remove('flex'); resetForm(); };
    const resetForm = () => {
        form.reset();
        form.action = '<?= $base ?>/admin/mata-kuliah';
    ...
    document.querySelectorAll('.btnEditMk').forEach(btn => {
        btn.addEventListener('click', () => {
            const id = btn.dataset.id;
            form.action = '<?= $base ?>/admin/mata-kuliah/' + id + '/update';
