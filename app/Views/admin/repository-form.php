...
$isEdit = ($mode ?? '') === 'edit';
$action = $isEdit
    ? "{$base}/admin/repository/" . ($item['id'] ?? '') . "/update"
    : "{$base}/admin/repository";
...
$mahasiswas = $mahasiswas ?? []; // users role mahasiswa
$dosens     = $dosens ?? [];     // users role dosen
...
<div class="space-y-6 animate-fade-in-up">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight leading-tight">
                <?= e($isEdit ? 'Edit Repository' : 'Tambah Repository') ?>
            </h1>
            <p class="text-sm text-slate-500 mt-1"><?= $isEdit ? 'Perbarui data repository' : 'Tambah repository baru' ?></p>
        </div>
        <div class="flex gap-2">
            <a href="<?= $base ?>/admin/repository" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-50 transition-colors">
    ...
    <form id="repoForm" action="<?= $action ?>" method="POST" enctype="multipart/form-data" class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="author" value="<?= e($item['author'] ?? '') ?>" required
                       ...
                       class="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-700">
                <p class="text-xs text-slate-400">Ini penulis versi teks.</p>
            </div>
            ...
            <div class="space-y-2">
                ...
                <select name="program_studi_id" class="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-700">
                    ...
                    <?php foreach ($prodis as $p): ?>
                        <option value="<?= $p['id'] ?>" <?= (!empty($item['program_studi_id']) && (int)$item['program_studi_id'] === (int)$p['id']) ? 'selected' : '' ?>>
            ...
            <div class="space-y-2">
                ...
                <select name="mata_kuliah_id" class="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-700">
                    ...
                    <?php foreach ($mks as $mk): ?>
                        <option value="<?= $mk['id'] ?>" <?= (!empty($item['mata_kuliah_id']) && (int)$item['mata_kuliah_id'] === (int)$mk['id']) ? 'selected' : '' ?>>
                            <?= e($mk['nama']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <!-- UPLOADED_BY + KEYWORDS -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-2">
                <label class="text-sm font-semibold text-slate-700">Uploader (opsional)</label>
                <select name="uploaded_by" class="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-700">
                    <option value="">Tidak ditetapkan</option>
                    <?php foreach ($authors as $author): ?>
                        <option value="<?= $author['id'] ?>" <?= (!empty($item['uploaded_by']) && (int)$item['uploaded_by'] === (int)$author['id']) ? 'selected' : '' ?>>
                            <?= e($author['nama_lengkap'] ?? $author['username'] ?? 'Pengguna') ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-2">
                ...
                <p class="text-xs text-slate-400">Dosen boleh ikut ditandai sebagai penulis, tapi minimal tetap ada 1 mahasiswa.</p>
        ...
        <div class="space-y-2">
            ...
            <textarea name="abstrak" rows="4" placeholder="Ringkasan singkat isi karya ilmiah"
                      class="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-700"><?= e($item['abstrak'] ?? '') ?></textarea>
        </div>
        ...
        <div class="flex flex-wrap justify-end gap-2 pt-2">
            <a href="<?= $base ?>/admin/repository" class="px-4 py-2.5 rounded-lg border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-colors">Batal</a>
            ...
            <?php if (!$isEdit): ?>
                <button type="submit" name="save_and_new" value="1" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-50 text-emerald-700 text-sm font-semibold hover:bg-emerald-100 border border-emerald-100 transition-all">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
...
document.getElementById('repoForm')?.addEventListener('submit', function(e){
  ...
  if (pickedStudents < 1) {
    e.preventDefault();
    alert('Minimal pilih 1 mahasiswa (Tandai Penulis).');
    ddToggle('author');
  }
});
</script>
